...
const sidebarItems: SidebarItem[] = [
  { name: 'Dashboard', path: '/business', icon: <LayoutDashboard className="w-6 h-6" /> },
  { name: 'Buy Credits', path: '/business/credits', icon: <CreditCard className="w-6 h-6" /> },
  { name: 'Bookings', path: '/business/bookings', icon: <Calendar className="w-6 h-6" /> },
  { name: 'Order History', path: '/business/orders', icon: <Receipt className="w-6 h-6" /> },
  { name: 'Settings', path: '/business/settings', icon: <Settings className="w-6 h-6" /> },
];
        ...
        {sidebarItems.map((item) => (
          ...
          >
            ...
            <motion.div
              initial={false}
              animate={{
                color: isActive(item.path) ? '#fbbf24' : '#9ca3af'
              }}
              className="flex-shrink-0 z-10"
            >
              {item.icon}
            </motion.div>
            <span className="font-medium text-base z-10">{item.name}</span>
          </Link>
        ))}
      </nav>
